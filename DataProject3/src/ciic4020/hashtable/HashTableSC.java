...
public class HashTableSC<K, V> implements Map<K, V> {
	...
	public void put(K key, V value) {
		...
		if(1.0*currentSize/buckets.length > loadFactor) rehash();
	...
	private void rehash() {
		...
		List<BucketNode<K, V>>[] temp = new List[buckets.length*2];

		for (int i = 0; i < temp.length; i++) {
			
			temp[i] = new LinkedList<BucketNode<K,V>>(); 
		}   
		for (int i =0; i < buckets.length; i++) {
			
			for(BucketNode<K, V> exe : buckets[i]) {
				int trg = hashFunction.hashCode(exe.key)%temp.length;
				
				List<BucketNode<K, V>> L =temp[trg];
				L.add(0, new BucketNode<K, V>(exe.key, exe.value));
